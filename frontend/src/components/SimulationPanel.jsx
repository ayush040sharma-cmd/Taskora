  ...
  return (
    <div className="sim-panel">
      <div className="sim-header">
        ...
        <p>Preview the impact of a task assignment before committing — see load changes and risk forecasts.</p>
      </div>
      ...
      <div className="sim-form">
        ...
        <div className="sim-field">
          <label>Estimated hours (optional override)</label>
          <input
            type="number"
            min="0.5"
            step="0.5"
            ...
            placeholder="Use task's default"
          />
        </div>
      ...
      {result && !result.error && (
        <div className="sim-result">
          <div className="sim-result-header">
            Simulation: assigning <strong>{result.task?.title}</strong> to <strong>{result.user?.name}</strong>
          </div>

          <div className="sim-result-feasibility" style={{
            background: result.simulation?.allowed ? "#d1fae5" : "#fee2e2",
            color: result.simulation?.allowed ? "#065f46" : "#991b1b",
          }}>
            {result.simulation?.allowed ? "✓ Feasible assignment" : "✗ Assignment not recommended"}
            {" — "}
            {result.simulation?.message || result.simulation?.reason}
          </div>

          <div className="sim-result-metrics">
            <div className="sim-metric">
              <div className="sim-metric-label">Load BEFORE</div>
              <LoadBar pct={result.simulation?.current_load_pct || 0} />
            </div>
            <div className="sim-metric">
              <div className="sim-metric-label">Load AFTER assignment</div>
              <LoadBar pct={result.simulation?.projected_load_pct || result.simulation?.new_load_pct || 0} />
            </div>
            {result.simulation?.delta_hours !== undefined && (
              <div className="sim-metric">
                <div className="sim-metric-label">Additional daily hours</div>
                <div className="sim-metric-val">+{result.simulation.delta_hours}h</div>
              </div>
            )}
          </div>
      ...
      {result?.error && (
        <div className="sim-error">✗ {result.error}</div>
      )}
          ...
          <div className="sim-suggestions-list">
            {suggestions.suggestions?.slice(0, 6).map((s, i) => (
              ...
              >
                ...
                {s.feasible && <span className="sim-sug-badge">✓ Recommended</span>}
              </div>
            ))}
          </div>
        </div>
